...
const BinManagement = () => {
    ...
    useEffect(() => {
        const getDrivers = async () => {
            try {
                const url = `${import.meta.env.VITE_SERVER_BASE_URL}/api/drivers`;
    ...
    }, []); // Refetch drivers when activeTab changes to ensure we have the latest data when viewing fleet management
    ...
    return (
        <div className="h-screen w-screen bg-slate-950 text-slate-100 flex flex-col font-sans overflow-hidden">
            ...
            <div className="flex flex-1 overflow-hidden">
                {/* SIDEBAR */}
                <aside className={`fixed inset-y-0 left-0 z-60 w-72 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out md:relative md:translate-x-0 md:flex md:w-64 flex-col bg-slate-900 border-r border-slate-800 shrink-0`} style={{ zIndex: 1000 }}>
