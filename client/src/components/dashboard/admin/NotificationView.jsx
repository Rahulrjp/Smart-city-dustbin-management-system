    ...
    return (
        <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
            <div className="flex justify-between items-center">
                ...
                <button className="text-xs font-bold text-emerald-400 hover:text-emerald-300 uppercase tracking-widest px-4 py-2 bg-emerald-500/10 rounded-xl border border-emerald-500/20">Mark All Read</button>
            </div>
                ...
                {notifications.map(n => (
                    <div key={n.id} className={`bg-slate-900 border ${!n.read ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-slate-800'} rounded-3xl p-6 transition-all`}>
                        <div className="flex gap-6 items-start">
                            <div className={`p-3 rounded-2xl ${n.type === 'warning' ? 'bg-amber-500/20 text-amber-400' : 'bg-blue-500/20 text-blue-400'}`}>
                                {n.type === 'warning' ? <ShieldAlert className="w-5 h-5" /> : <Info className="w-5 h-5" />}
                            </div>
